...
public class ObjectManager {
    ...
    private void drawWaterAni(Graphics g, int xLvlOffset) {
        for (WaterTop w : waterTop) {
            int type = 0;
            if (w.getObjType() == Constants.ObjectConstants.WATER_TOP2)
                type = 1;
            if (w.getObjType() == Constants.ObjectConstants.WATER_TOP3)
                type = 2;
            g.drawImage(waterTopImg[type][w.getAniIndex()], (int) (w.getHitbox().x - w.getxDrawOffset() - xLvlOffset),
                    (int) (w.getHitbox().y - w.getyDrawOffset()), Constants.ObjectConstants.WATER_DIMENSIONS, Constants.ObjectConstants.WATER_DIMENSIONS, null);
        }
    }
